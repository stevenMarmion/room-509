<template>
  <div class="admin-section">
    ...
    <div class="admin-toolbar">
      <input v-model="search" class="admin-search" placeholder="Search friendships…" />
    </div>
    <div v-if="loading" class="admin-state">Loading…</div>
    <div v-else class="admin-table-wrap">
      <table class="admin-table">
        <thead><tr><th>ID</th><th>Requester</th><th>Addressee</th><th>Status</th><th>Since</th><th>Actions</th></tr></thead>
        <tbody>
          <tr v-for="f in filtered" :key="f.id">
            <td class="td-muted">{{ f.id }}</td>
            <td class="td-bold">{{ f.requesterPseudo ?? f.requester?.pseudo ?? '—' }}</td>
            <td>{{ f.addresseePseudo ?? f.addressee?.pseudo ?? '—' }}</td>
            <td>
...
const filtered = computed(() => {
  ...
  return items.value.filter(f =>
    f.requesterPseudo?.toLowerCase().includes(q) ||
    f.addresseePseudo?.toLowerCase().includes(q) ||
    f.requester?.pseudo?.toLowerCase().includes(q) ||
    f.addressee?.pseudo?.toLowerCase().includes(q)
  )
})
async function load() {
  loading.value = true
...
async function remove(id) {
  ...
  try { await delete_api(`/api/friendships/${id}`); items.value = items.value.filter(f => f.id !== id); showToast('Deleted') }
